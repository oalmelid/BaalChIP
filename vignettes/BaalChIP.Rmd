...
**(c)** The second module of BaalChIP consists of calling ASB binding events. (4) BaalChIP uses a beta-binomial Bayesian model to consider RM and RAF bias when detecting the ASB events. **(d)** The output from BaalChIP is a posterior distribution for each SNP. A threshold to identify SNPs with allelic bias is specified by the user (default value is a 95\% interval). (5) The output of BaalChIP is a credible interval ('Lower' and 'Upper') calculated based on the posterior distribution. This interval corresponds to the true allelic ratio in read counts  (i.e. after correcting for RM and RAF biases). An ASB event is called if the lower and upper limits of the interval are outside the 0.4-0.6 interval.
...
```{r, eval=FALSE}
setwd(system.file("test",package="BaalChIP")) #set wd

samplesheet <- "exampleChIP1.tsv"
...
```{r quick1, eval=FALSE}
res <- new("BaalChIP", samplesheet=samplesheet, hets=hets)
res <- BaalChIP.run(res, cores=4) #cores for parallel computing
```
...
The second dataset contains ChIP-seq and genomic DNA (gDNA) data obtained from two cancer cell lines. In this example, the allelic-ratios obtained from the sequenced gDNA samples are used to correct the allelic read counts for biases caused by copy number alterations.
...
```{r}
gDNA <- list("TaT1"=
               c("bamFiles/TaT1_1_gDNA.test.bam",
                 "bamFiles/TaT1_2_gDNA.test.bam"),
              "AMOVC"=
               c("bamFiles/AMOVC_1_gDNA.test.bam",
                 "bamFiles/AMOVC_2_gDNA.test.bam"))
```
...
samplesheet <- "exampleChIP2.tsv"
hets <- c("TaT1"="TaT1_hetSNP.txt", "AMOVC"="AMOVC_hetSNP.txt")
res2 <- new("BaalChIP", samplesheet=samplesheet, hets=hets, CorrectWithgDNA=gDNA)
res2 <- BaalChIP.run(res2, cores=4) #cores for parallel computing
```
...
```{r}
samplesheet <- read.delim(file.path(system.file("test",package="BaalChIP"),"exampleChIP1.tsv"))
samplesheet
```
...
```{r}
samplesheet <- read.delim(file.path(system.file("test",package="BaalChIP"),"exampleChIP2.tsv"))
samplesheet
```

This sample sheet details the metadata for ChIP-seq studies in TaT1 and AMOVC cell lines. For each study, ChIP-seq data exists for one transcription factors and 2 replicates each (hence, 4 BAM files). 
...
4. The final column `RAF` is optional. RAF scores consist of values ranging from 0 to 1 for each variant denoting the reference allele frequency. A value between 0.5 and 1 denotes a bias to the reference allele, and a value between 0 and 0.5 a bias to the alternate allele. This column is optional, and will not be necessary if ask BaalChIP to calculate the RAF values from the input gDNA libraries (by using `CorrectWithgDNA` argument of the BaalChIP-class constructor function `new` - see below). 

### The gDNA BAM files

The gDNA BAM files are input genomic DNA sequencing files for the corresponding groups in the ChIP-seq test data. These BAM files are passed to BaalChIP through the `CorrectWithgDNA` argument of the BaalChIP-class constructor function `new`.
...
The first step is to generate a BaalChIP object. The function `new` accepts the following arguments: 
...
1. `blacklist_hg19` contains blacklisted genomic regions downloaded from the UCSC Genome Browser [@fujita2010ucsc] (mappability track; hg19, wgEncodeDacMapabilityConsensusExcludable and wgEncodeDukeMapabilityRegionsExcludable tables). These regions should be used as `RegionsToFilter` so that variants overlapping these regions will be removed.
...
3. Simulated allelic read counts are computed using the `pileup` function of the `r Biocpkg("Rsamtools")` package [@morganrsamtools]. 
4. Eliminates SNPs with incorrect number of read alignments (corresponds to twice the read length for each allele). 
...
```{r, eval=FALSE}
res <- filterIntbias(res, simul_output="directory_name", tmpfile_prefix="prefix_name", simulation_script = "local")
```
...
2. `tmpfile_prefix` argument is an optional character vector giving the initial part of the name of the FASTQ and BAM files generated by the function. If NULL or missing, a random prefix name will be generated.
3. `simulation_script` the file path for simulation script containing the instructions of simulation and alignment commands. If set to 'local', the default simulation script distributed with BaalChIP ('extra/simulation_run.sh') will be used. 
...
By specifying `skipScriptRun=TRUE`, BaalChIP will take the simulated reads allelic ratios directly from the pre-computed simulated datasets and use them for further filtering (the simulation (step 1) and alignment (step 2) will be skipped).

For this particular example we can run:

```{r, eval=FALSE}
res <- filterIntbias(res, simul_output=preComputed_output, tmpfile_prefix="c67c6ec6c433", skipScriptRun=TRUE)
```
...
The reference mapping bias is calculated as described in [@kilpinen2013coordinated] and [@lappalainen2013transcriptome]. 

This bias correction can be turned on/off by using the argument `RMcorrection=FALSE` of the `getASB` function
...
This bias correction can be turned on/off by using the argument `RAFcorrection=FALSE` of the `getASB` function

You can also access the final `biasTable` with the estimated RM and RAF scores per variant and per group_name:

```{r}
biastable <- BaalChIP.get(res, "biasTable")
head(biastable[["MCF7"]])
```
...
The `average_stats` shows that on average `r round(a[[2]]["pass","perc"],2)`% of SNPs pass all filters, meaning that BaalChIP removed an average of `r round(100-a[[2]]["pass","perc"],2)`% of all SNPs (with slight variation between cell lines).
...
The function `plotSimul` produces a plot of the proportion of SNPs that displayed the correct number of mapped simulated reads for the different read lengths considered in the ENCODE experiment (28mer to 50mer). The percentage of correct calls increases with the sequencing length of the simulated reads.
...
We identified a total of 3208 and 1624 heterozygous SNPs in MDA-MB-134 and T-47D cells, respectively. 
...
We observed that after RAF correction the allelic ratios become more evenly distributed around an average of 0.5. 
