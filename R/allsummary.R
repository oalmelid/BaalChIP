...
get_filterstats <- function(sampledata, mergeddata = NULL) {
    ...
    if (!is.null(mergeddata)) {
        allcounts <- c(allcounts, sapply(mergeddata, nrow))
    }
    ...
    total <- allcounts[1]
    if (sum(nrfilt) != total) {
        stop("error counting filtered SNPs", call. = FALSE)
    }
...
applyFilteringStats <- function(res_per_bam, res_merged) {
    ...
    filtering_stats <- lapply(names(res_per_bam), function(cellname) {
        ...
        if (!is.null(res_merged)) {
            mergeddata <- res_merged[[cellname]]
        } else {
            mergeddata <- NULL
        }
        if (all(sapply(mergeddata, length) == 0)) {
            mergeddata <- NULL
        }
        get_filterstats(sampledata, mergeddata)
    })

    if (all(sapply(filtering_stats, length) == 0)) {
        return(list())
    }
...
summary_QC <- function(object) {
    ...
    if (length(mergedCounts) == 0) {
        res_merged <- NULL
    } else {
        res_merged <- mergedCounts
    }
    if (length(alleleCounts) == 0) {
        res_per_bam <- NULL
    } else {
        res_per_bam <- alleleCounts
    }
...
summary_ASB <- function(object) {
    ...
    asb_stats <- do.call("rbind", lapply(asb, function(x) {
        c("Ref" = sum(x$Bayes_sig_A), "Alt" = sum(x$Bayes_sig_B))
    }))
...
getReport <- function(object, group_name) {
    ...
    correctedAR <- asb[!names(asb) %in% c("Bayes_sig_A", "Bayes_sig_B")]
    correctedAR["Corrected.AR"] <- rowMeans(correctedAR[, c("Bayes_lower", "Bayes_upper")])
    correctedAR["isASB"] <- as.character(asb[, "Bayes_sig_A"] == 1 | asb[, "Bayes_sig_B"] == 1)
