...
Sum_read_counts <- function(SNP_hit_Peaks) {
  ...
  for (id_col in 1:length(peak_id)) {
    ...
    if (length(id_REF) ==1) {
        SNP_hit_Peaks_sum <- cbind(SNP_hit_Peaks_sum, SNP_hit_Peaks[,peak_id[id_col]], SNP_hit_Peaks[,id_REF], rowSums(SNP_hit_Peaks[,id_TOT], na.rm= T))
    } else {
        SNP_hit_Peaks_sum <- cbind(SNP_hit_Peaks_sum, SNP_hit_Peaks[,peak_id[id_col]], rowSums(SNP_hit_Peaks[,id_REF], na.rm= T), rowSums(SNP_hit_Peaks[,id_TOT], na.rm= T))
  }
...
runBayes <- function(counts, bias, Iter=5000, conf_level=0.99)
{
    ...
    cores = 4 # how many cores to use parallelly
